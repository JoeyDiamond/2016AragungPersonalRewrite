...
public class ShootOuterworksShot extends Action{

	@Override
	public void run() {
		System.out.println("ShootOuterworksShot: starting");
		...
		while((!shooter.isInPosition() || !global.shotReadyToFire) && !global.cancelAutomation){
			shooter.setTurretHoodFlywheelKick(global.manualShotTurretAngle,
					Constants.HOOD_OUTER_SHOT_ANGLE,
					Constants.FLYWHEEL_OUTER_SHOT_SPEED,
					false);
		...
		System.out.println("ShootOuterworksShot: fire shot");
		new FireShotAndStore().run();
		
		global.automated = false;
		System.out.println("ShootOuterworksShot: finished");
